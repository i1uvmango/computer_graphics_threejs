...
var endQuat    = new THREE.Quaternion(); // identity — init()에서 endQuat.identity()
...
function createScene() {
    ...
    // 카메라를 cameraHolder의 자식으로 연결 — 비행기 follow 시 twitching 없음
    cameraHolder = new THREE.Object3D();
    scene.add(cameraHolder);
    cameraHolder.add(camera);
    camera.position.set(0, 0, 200); // 기본: 비행기 뒤 200 유닛
...
// ==================== updatePlane: LERP + 2-phase SLERP ====================
function updatePlane() {
    // --- Per-frame movement while key held ---
    const SPEED = 2.0;
    ...
    // --- Translation: lerp 0.1 비율로 targetPos 추종 ---
    ...
    // 2-phase slerp: 키 누르는 동안 banking, 뗄 때 leveling
    const wasdHeld = keysHeld['KeyW'] || keysHeld['KeyS'] || keysHeld['KeyA'] || keysHeld['KeyD'];
    if (wasdHeld) {
        // 1단계 BANKING: startQuat → targetQuat
        ...
        // 2단계 LEVELING: midQuat → endQuat(identity)
...
function updateCamera() {
    // FOV 보간 — 위치/자세 보간과 독립적으로 매 프레임 실행
    ...
    if (cameraMode === 'fps') {
        ...
        // lookAt을 직접 호출하면 rotation이 snap → 목표 quaternion 계산 후 slerp
